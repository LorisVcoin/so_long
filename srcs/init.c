...
/*   Updated: 2025/03/04 14:49:24 by lviravon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_window(t_init *init)
{
	...
	init->info.title = "So long";
	init->info.width = 1920;
	init->info.height = 1080;
	init->win = mlx_new_window(init->mlx, &init->info);
}

void	init_map(t_init init, int fd, t_images *textures, t_move *move)
{
	static int	i = 0;
	...
	aff_exit(init, textures, init.map);
}
